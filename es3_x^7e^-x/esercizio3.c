/*
   gcc -o esercizio3.exe esercizio3.c -lm && ./esercizio3.exe
*/
...
long double min = 0; // estremi integrazione
long double max = 5;
...
int main()
{
   long double exact = 5040. - 648240. * expl(-5);
   ...
   {
      laguerre = gauss_laguerre_finito(lag[i].n, f_laguerre, min, max);
...
{
   long double y = powl(x, 7) * expl(-x);
   return y;
}

long double f_laguerre(long double x)
{
   long double y = powl(x, 7);
   return y;
}